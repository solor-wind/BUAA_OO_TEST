    ...
    for i in range(len(lines)):
        ...
        try:
            if matcher.group(2) in ['ARRIVE','OPEN','CLOSE','RECEIVE']:
                ...
                actions.append([float(matcher.group(1)),matcher.group(2),int(matcher.group(3)),int(matcher.group(4)),int(matcher.group(5))])
            elif matcher.group(2)=='RESET_ACCEPT':
                matcher=re.match(r'\[\s*(\d+\.\d+)\]([A-Z_]+)-(\d+)-(\d+)-(\d+\.\d+)',line)
                actions.append([float(matcher.group(1)),matcher.group(2),int(matcher.group(3)),int(matcher.group(4)),float(matcher.group(5))])
