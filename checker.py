...
class Checker:
    ...
    def check_send_message(self, order) -> str:
        ...
        if message.type == 1 and message.person_id2_or_tag_id not in person1.tags.keys():
            self.exception["tinf"] += 1
            if message.person_id2_or_tag_id in self.tinfId2Num.keys():
                self.tinfId2Num[message.person_id2_or_tag_id] += 1
            else:
                self.tinfId2Num[message.person_id2_or_tag_id] = 1
            return f"tinf-{self.exception['tinf']}, {message.person_id2_or_tag_id}-{self.tinfId2Num[message.person_id2_or_tag_id]}"
        if message.type == 0:
            ...
            tag = person1.tags[message.person_id2_or_tag_id]
            del self.id2Message[messageId]
            person1.add_social_value(message.social_value)
